...
export async function GithubInfo({
  ...
  return (
    <a
      ...
      {...props}
      className={cn(
        "flex flex-col gap-1.5 rounded-lg p-2 text-fd-foreground/80 text-sm transition-colors hover:bg-fd-accent hover:text-fd-accent-foreground lg:flex-row lg:items-center",
        props.className
      )}
    >
      ...
      <p className="flex items-center gap-1 text-fd-muted-foreground text-xs">
...
function humanizeNumber(num: number): string {
  ...
  if (num < HUNDRED_THOUSAND) {
    ...
    const value = (num / THOUSAND).toFixed(1);
