...
- **Self-hosted Kubernetes**: `kit/charts/atk` packages the dApp, ORPC backend, Besu network, Blockscout, Hasura, MinIO, and observability stack with environment-specific values, disruption budgets, and network policies.
...
- The dApp and ORPC backend already model OAuth flows, MFA checks, and granular role validation, so existing identity providers plug straight in.
...
- Moving between models is configuration work: Compose for development, Helm for self-managed clusters, and Console for managed service all share contracts, schema, and ORPC procedures.
