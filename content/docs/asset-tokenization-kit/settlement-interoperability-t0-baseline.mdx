...
<Accordions type="single" defaultValue="confirmation">
  <Accordion title="Bank confirmation stalls" value="confirmation">
    <p>
  ...
  <Accordion title="Policy mismatch" value="policy">
    <p>
      Bridges refuse to mint on destinations that miss policy checks or limits. The validator blocks the hop immediately and logs the rejection for compliance review instead of leaving the trade in limbo.
    </p>
  </Accordion>
  <Accordion title="Ledger vs. bank drift" value="ledger">
    <p>
      If the ledger and bank view diverge, the ISO 20022 message flow becomes the system of record for booking and reconciliation, eliminating ambiguity about settlement state.
    </p>
  </Accordion>
</Accordions>
