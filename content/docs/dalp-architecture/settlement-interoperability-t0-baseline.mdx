...
<Accordions type="single" defaultValue="bank-confirmation">
  <Accordion title="Bank confirmation stalls" value="bank-confirmation">
    <p>
  ...
  <Accordion title="Policy mismatch across bridges" value="policy-mismatch">
    <p>
      Bridges refuse to mint or release assets on destinations that fail policy or limit checks. Rather than forcing an operator to discover the issue later, the validator blocks the hop immediately and logs the rejection for compliance review.
    </p>
  </Accordion>
  <Accordion title="Ledger discrepancies" value="ledger-gap">
    <p>
      When on-chain events disagree with bank ledgers, the ISO 20022 message flow becomes the authoritative record. Booking and reconciliation teams work from the same dataset, so there is no ambiguity about the state of a transfer.
    </p>
  </Accordion>
</Accordions>
