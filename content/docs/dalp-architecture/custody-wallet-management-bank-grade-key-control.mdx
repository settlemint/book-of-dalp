...
flowchart LR
  ...
  subgraph Execution
    queue["Approval queue (multi-sig)"]
    signer["Custody signer service"]
    rails["Custodian APIs & payment rails"]
  end
  subgraph Transparency
    attest["Omnibus attestations"]
    cap["Cap-table indexer"]
    compliance["Compliance engine"]
  end

  handshake --> policies --> queue --> signer --> rails
  rails --> notifier["Asset Manager dashboards"]
  rails --> audit["Audit logs & SLA metrics"]
  rails --> attest --> cap --> compliance
  compliance --> queue
`}
/>
