...
- **Self-hosted Kubernetes**: The `kit/charts/atk` Helm stack packages the entire platform (dApp, REST API backend, Hyperledger Besu network, Blockscout, Hasura, MinIO, observability) with environment-specific `values*.yaml` files, pod disruption budgets, and network policies. You install it into your cluster and keep full control over nodes, keys, and change management.
